...
_RESULT_SPECS = [
    ...
    ResultSpec(
        "AutoGen Native Memory",
        "autogen_python/native_memory/results.json",
        "external",
    ),
    ResultSpec(
        "ADK Native Memory",
    ...
    ResultSpec(
        ...
        "crewai_python/native_memory/results.json",
        "external",
    ),
]
...
def render(results: list[ResultView], zh: bool) -> str:
    ...
    if zh:
        ...
        lines.append("- **本轮 memory/native 对比**：`trpc-agent-go auto`、AutoGen、ADK、Agno、CrewAI。")
        ...
        lines.append("- **Memory/native runs**: `trpc-agent-go auto`, AutoGen, ADK, Agno, and CrewAI.")
...
def _append_fairness_section(lines: list[str], zh: bool) -> None:
    if zh:
        ...
        lines.append("- **外部框架**：AutoGen、ADK、Agno、CrewAI 使用各自 native memory 能力；memory 表示、抽取策略、向量库和工具暴露方式可能不同。")
        ...
        lines.append("- **External frameworks**: AutoGen, ADK, Agno, and CrewAI use their native memory capabilities; memory representation, extraction strategy, vector store, and tool exposure may differ.")
