...
func main() {
	...
	cfg.ModeName = "react + ralph-loop"
	cfg.Framework = "trpc-agent-go-ralph"
	cfg.OutputPath = "../results/trpc-agent-go_ralph.json"
	maxIterations := 3
	...
	flag.IntVar(&maxIterations, "max-iterations", maxIterations, "Max Ralph Loop iterations")
	flag.Parse()

	cfg.RunnerFactory = gaiaeval.RalphRunnerFactory(maxIterations)
	if err := gaiaeval.Run(context.Background(), cfg); err != nil {
		log.Fatalf("run Ralph benchmark: %v", err)
	}
}
